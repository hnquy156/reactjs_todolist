...
function Sort(props) {
    ...
    function handleSort(orderBy, orderDirection) {
        props.onClickSort(orderBy, orderDirection);
    }

    let { orderBy, orderDirection } = props;
    let sortStr = orderBy + " - " + orderDirection;
    return (
        <div className="col-xs-3 col-sm-3 col-md-3 col-lg-3">
            <div className="dropdown">
                ...
                <span className="label label-success label-medium">{ sortStr.toUpperCase() }</span>
            </div>
        </div>
    );
}
